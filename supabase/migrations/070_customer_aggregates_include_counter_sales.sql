-- Vendas registradas no balcão também são compras do cliente e entram nos agregados.
begin;
...
as $$
with order_agg as (
  select coalesce(delivered.pessoa_id, counter.pessoa_id) pessoa_id,
         (coalesce(delivered.total_orders, 0) + coalesce(counter.total_orders, 0))::bigint total_orders,
         (coalesce(delivered.total_value, 0) + coalesce(counter.total_value, 0))::numeric total_value,
         case
           when delivered.last_order_at is null then counter.last_order_at
           when counter.last_order_at is null then delivered.last_order_at
           else greatest(delivered.last_order_at, counter.last_order_at)
         end last_delivered_at
    from (
      select o.pessoa_id,
             count(*)::bigint total_orders,
             coalesce(sum(o.total), 0)::numeric total_value,
             max(coalesce(o.closed_at, o.created_at)) last_order_at
    ...
    full outer join (
      select coalesce(v.id_cliente, v.id_pessoa) pessoa_id,
             count(*)::bigint total_orders,
             coalesce(sum(v.valor_total), 0)::numeric total_value,
             max(v.data_hora) last_order_at
        from public.vendas v
       where coalesce(v.id_cliente, v.id_pessoa) is not null
         and v.id_usuario = p_owner_user_id
       group by coalesce(v.id_cliente, v.id_pessoa)
    ) counter on counter.pessoa_id = delivered.pessoa_id
), conversation_agg as (
  select s.pessoa_id,
         max(case
