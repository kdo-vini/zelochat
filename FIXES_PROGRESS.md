...
- ✅ ZCHAT-IO-002 — o sweeper fazia deletes amplos concorrentes e `processed_at` caía em `Seq Scan` → migration aditiva aplicada em produção cria índices concorrentes e RPC service-role de lotes com lock/`SKIP LOCKED`; o worker drena lotes de 500 serialmente, com pausa, mantendo 7 dias de sucessos e 21 de falhas. ACL validada (`service_role=true`, browser roles=false) e `EXPLAIN` confirmou `Index Only Scan` — `supabase/migrations/20260918044906_optimize_webhook_raw_io.sql`, `server/webhookEventsSweeper.ts`, `tests/webhookEventsSweeper.test.ts`
- ✅ ZCHAT-IO-003 — redelivery atualizava sessão antes da dedupe, cada inbound novo fazia dois updates e operações no-op regravavam `updated_at`, coluna presente em oito índices → dedupe antecipada, preview+número de não lidos em uma RPC pós-insert, patch somente de campos divergentes, guards para lida/foto/archive/pin/nome e remoção concorrente do índice duplicado — `server/messageHandler.ts`, `supabase/migrations/20260918044906_optimize_webhook_raw_io.sql`, `tests/sessionWriteReduction.test.ts`

## Rastro da IA nunca gravava nenhuma linha — 2026-09-15

- ✅ ZCHAT-OBS-003 — `zelochat_ai_turn_traces` (ZCHAT-OBS-001) estava vazia desde que subiu em 09/09: o turno do modelo passava `sessionId: session.id`, mas `StoredSession.id` é o JID (`mapSession` monta a família de sessões com `id: canonicalRow.remote_jid`, não a UUID da linha) → todo insert falhava com Postgres `22P02 invalid input syntax for type uuid`, engolido em silêncio pelo catch que existe pra nunca custar a resposta ao cliente. Achado revisando o relatório diário da Bem Servido: a tabela estava zerada até para os dois turnos de modelo do dia. `remoteJid` já identifica a conversa, então o campo foi só removido em vez de plumbed até a UUID real — `server/ai.ts:4038`, `tests/aiTurnTrace.test.ts`. Publicado pelo auto-deploy normal do Dokploy (`deployment.all` mostra o build automático começando ~57s depois do push, commit `b1da535`, `done`); um deploy manual redundante foi disparado por engano por causa de um falso negativo — `application.one` só traz uma lista curta/desatualizada de deployments e não mostrava o build automático já em andamento. Sem efeito colateral (mesmo commit, os dois `done`). Ver [[CURRENT]].
...
Fecha os itens de prontidão pré-piloto do `ultra-review-production.md` (PR 1.1, 1.3, 1.5, 1.6, 1.7, 1.30, 1.31, 1.39, I-5, I-6, I-15) que a wave de correções C1-C6 (ver seção abaixo) não cobria.

- ↩️ PR-1.1/1.3 (superado no mesmo dia por AI-WA-ROLLOUT-REMOVE) — a primeira contenção criou `empresa_perfil.ai_hybrid_ordering_enabled`, migration 068 e gates por tenant; a decisão de produto posterior retirou esse rollout gradual e preservou somente modo restaurante + controles globais da IA. Registro histórico; esses símbolos/arquivos não existem mais.
- ✅ PR-I-15 — `storeOpen` do cartão de entrada vinha só do horário semanal; numa data bloqueada (`blocked_dates`) uma saudação recebia o botão "Pedir por aqui" ativo mesmo com a loja fechada → `resolveOrderingEntryStoreOpen()` fecha a entrada canônica na mesma condição que já bloqueia `criar_pedido`/retirada/Pix para datas bloqueadas — `server/ai.ts:1092`, `tests/aiOrderingEntryBlockedDates.test.ts:1`
- ✅ PR-1.6/I-6 — `ZeloMenuInternalClient.fromEnv` validava só a API key; base URL ausente/errada em produção caía silenciosamente em `127.0.0.1:3101` e toda chamada canônica dava ECONNREFUSED sem log nenhum apontando a causa → em ambiente `production-like` (NODE_ENV ou PUBLIC_APP_URL público), chave presente + base URL ausente agora loga um código estruturado uma única vez e desativa o caminho canônico em vez de adivinhar; dev/local sem sinal continua usando o default local — `server/zeloMenuInternalClient.ts:194`, `tests/zeloMenuInternalClientEnv.test.ts:1`
- ✅ PR-1.5 — env vars novas (`ZELOMENU_INTERNAL_*`, `AUDIO_TRANSCRIPTION_*`) sem validação de startup → uma linha de log única no boot lista presença/ausência de cada uma (nunca o valor) — `server/index.ts:440`, CLAUDE.md §Environment
- ✅ PR-I-5 — indisponibilidade do ZeloMenu (503/429/5xx) virava escalação para humano sem limite algum: numa instabilidade, toda conversa aberta escalava independentemente e o WhatsApp do dono enchia de aviso de manager → circuit breaker por empresa (`server/orderingCircuitBreaker.ts`): 5 falhas de transporte consecutivas em 60s abrem o circuito por 30s; enquanto aberto, o cliente falha rápido (sem nova tentativa de rede) e o cliente recebe só o aviso de "tente de novo"; UMA notificação de manager por abertura, não por conversa; falha de circuito nunca gasta o orçamento de tentativas por conversa (`countsTowardLimit:false`) — `server/orderingCircuitBreaker.ts:1`, `server/zeloMenuInternalClient.ts:184`, `server/aiWhatsAppOrdering.ts:93`, `tests/orderingCircuitBreaker.test.ts:1`, `tests/zeloMenuInternalClient.test.ts:1`
- ✅ IA-WA-021 — `CONFIRMACAO_INDISPONIVEL` é falha de disponibilidade do serviço de confirmação da loja (RPC ou segredo do token ausente — exatamente um deploy fora de lockstep entre os dois repos), mas vinha como HTTP 400: o client resetava o breaker em todo 4xx e o classificador caía em `escalate`, então toda confirmação escalava e notificava o gerente uma vez POR CONVERSA — o mesmo flood que o breaker do PR I-5 existe pra impedir. Além disso, snapshot pronto sem `confirmationAction.token` ainda enviava um `confirm_draft` garantidamente rejeitado como `COMANDO_INVALIDO`, gastando um handoff humano → esse código 400 específico (e só ele) agora conta como falha do breaker; `CONFIRMACAO_INDISPONIVEL` e o código local novo `CONFIRMACAO_SEM_TOKEN` usam retry amigável com `countsTowardLimit:false`; a confirmação sem token falha localmente depois das guardas de revisão e permit, com zero mutações. Os dois códigos são distintos de propósito para a métrica separar "faltou token no snapshot" de "serviço da loja fora" — `server/zeloMenuInternalClient.ts:298`, `server/aiWhatsAppOrdering.ts:138`, `server/aiWhatsAppOrdering.ts:685`, `tests/zeloMenuInternalClient.test.ts:179`, `tests/hybridOrderingScenarios.test.ts:708`
- ✅ PR-1.30/1.31 — métrica de pedido só tinha `{event, outcome, durationMs}`, sem correlação de conversa/tenant nenhuma, e o split "canônico atendeu vs. caiu pro modelo genérico" não era logado → `buildOrderingMetricLine` estrutura cada linha com `empresaId`, uma chave de conversa hasheada (sha256 do JID, 12 hex — nunca o JID cru), `orderingId`, `revision`, `stage` (um dos 10 estágios canônicos), `outcome`, `errorCode`, `durationMs`; toda saída `handled:false` do router canônico agora loga `stage:'plan', outcome:'fell_through_generic'` — `server/aiWhatsAppOrdering.ts:215`, `tests/aiOrderingMetrics.test.ts:1`
- ✅ PR-1.39 — Novidades publicava a entrada `big` "Pedido por mensagem ficou mais fácil" antes de qualquer validação real ponta a ponta → entrada removida de `src/data/changelog.ts`; volta somente depois da primeira ordem real confirmada, independentemente da retirada posterior do rollout por tenant — `src/data/changelog.ts:17`

### Pedidos conversacionais híbridos — simplificação por decisão de produto (2026-09-04)

- ✅ AI-WA-ROLLOUT-REMOVE — havia uma chave por empresa, migration, hidratação isolada, gates duplicados e um terceiro estado na interface para um piloto que nunca foi solicitado; como existe um cliente em produção e não haverá rollout gradual, o fluxo canônico volta a rodar para toda empresa fora do modo `general`, mantendo `ai_enabled` e `ai_mode = 'always_off'` como controles operacionais → migration 068 e `tests/aiHybridOrderingFlag.test.ts:1` removidos; runtime/status simplificados em `server/configStore.ts:68`, `server/ai.ts:3786`, `server/router.ts:634`, `server/aiSimulator.ts:140` e `src/services/waApi.ts:43`; card reduzido a integração configurada/não configurada em `src/domain/aiWhatsAppOrderingUi.ts:14` e `src/components/views/SettingsView.tsx:483`; cobertura refeita em `tests/aiHybridOrderingRestaurantMode.test.ts:8`, `tests/aiSimulatorOrdering.test.ts:74`, `tests/aiWhatsAppOrderingStatusRoute.test.ts:6`, `tests/aiWhatsAppOrderingUi.test.ts:4`, `tests/hybridOrderingScenarios.test.ts:399` e `tests/routerWebhookGuardrails.test.ts:108`; documentação atualizada em `CLAUDE.md:232`, `CURRENT.md:16` e `docs/ai/ZeloChat.memory.md:14`.
- ✅ AI-WA-ROLLOUT-GUARDS — a retirada da flag também havia retirado a única guarda dos caminhos diretos de botão e de Cancelar/Alterar digitados, permitindo mutação canônica em modo `general` ou com a IA global desligada → `isCanonicalOrderingAllowedNow()` centraliza modo + agenda/kill switch, e o webhook valida essa política antes de persistir, adquirir permit ou mutar; regressão cobre `general`, `always_off`, `ai_enabled=false` e a ordem das guardas — `server/configStore.ts:493`, `server/router.ts:648`, `server/router.ts:731`, `tests/aiHybridOrderingRestaurantMode.test.ts:1`

### Pedidos conversacionais híbridos — contrato real com a autoridade (2026-09-04)

Fecha os achados de `ultra-review-contract.md` (10 Critical / 11 Important), `ultra-review-functional.md` (C1–C5, I1–I7) e a parte de integridade de `ultra-review-production.md` (C-1 a C-7). Causa-raiz comum: cada repositório testava contra um double inventado do outro, então a suíte ficava verde enquanto o formato real divergia. Dependência cross-repo: ZeloMenu `kdo-vini/ai-ordering-authority` commits `9b48233` (fixtures gravadas), `89c1aff` (limiter por empresa), `2f090df` (token de confirmação obrigatório).

- ✅ IA-WA-014 — o ZeloChat lia `requirements[]` como `kind`/`label` enquanto a autoridade emite `type`/`name` e usa `kind` para o subtipo do modificador (`adicional`/`variacao`); todo requisito renderizava vazio e caía em escalação, taps de modificador não parseavam (o ID do requisito contém `:`), rótulos com preço estouravam os limites de 20/24 caracteres e um snapshot sem `requirements` estourava `TypeError` → `parseOrderingSnapshotWire` é o único ponto de tradução do formato da autoridade e falha fechado com `ORDERING_WIRE_UNSUPPORTED`; o apresentador virou um mapa exaustivo (`satisfies Record<...>`, tipo novo sem branch quebra o typecheck), preço vai para o corpo/descrição e nunca no título, opcionais listam todas as opções; IDs de botão passam a `REQ:<requisito>|<opção>|<fingerprint de orderingId:revision>` — `server/zeloMenuOrderingWire.ts:146`, `src/domain/orderingRequirementPresenter.ts:1`, `server/zeloMenuInternalClient.ts:1`, `tests/zeloMenuWireContract.test.ts:1`
- ✅ IA-WA-015 — os corpos enviados eram rejeitados pela autoridade: `snapshotToDraft` devolvia `deliveryFee`/`deliveryFeeToConfirm` e `fulfillment.type: null`, o nome do cliente virava `null` a cada update (reabrindo um requisito bloqueante para sempre), o "de sempre" não tinha `lineId`, um patch só de quantidade apagava os modificadores da linha, `removedLineIds` nunca era enviado e a confirmação por texto não levava `confirmationToken` → sanitizadores de fulfillment/cliente, merge por `lineId` que preserva seleções, `removedLineIds` ponta a ponta, `lineId` no pedido repetido, guarda de profundidade na auto-seleção (`MAX_AUTO_SELECT_DEPTH`) e token em toda confirmação — `src/domain/aiWhatsAppOrdering.ts:713`, `server/orderingPatchPlanner.ts:1`, `server/aiWhatsAppOrdering.ts:1`, `tests/orderingPatchPlanner.test.ts:1`
- ✅ IA-WA-016 — todo código de erro da autoridade caía num único catch que escalava para humano, inclusive `AI_TURN_REVOKED` (deveria ser silencioso) e os 409 que carregam o snapshot `current` (ponteiro perdido travava a conversa para sempre); o timeout de 4 s valia até para o confirm, que faz várias idas ao banco → `classifyOrderingFailure` mapeia cada código de `errors.json` em `suppress`/`resync`/`clear_and_tell`/`retry_later`/`escalate`, o confirm tem timeout próprio e reconcilia por consulta antes de dizer que falhou, e mensagem interna de revalidação não vai mais literal para o cliente — `server/aiWhatsAppOrdering.ts:113`, `server/zeloMenuInternalClient.ts:1`, `tests/aiWhatsAppOrdering.test.ts:1`
- ✅ IA-WA-017 — "confirmar"/"cancelar" digitados com rascunho canônico aberto batiam na camada legada de botão e respondiam "Seu pedido já foi confirmado! ✅" sem criar nada (ou silêncio, com o rascunho aberto) → com ponteiro canônico presente, confirmar/alterar/cancelar vão para o fluxo canônico; sem ponteiro e sem pendência legada, a resposta idempotente legada continua idêntica — `server/router.ts:696`, `tests/orderConfirmationPipeline.test.ts:1`, `tests/routerWebhookGuardrails.test.ts:1`
- ✅ IA-WA-018 — a confirmação podia fechar uma revisão que o cliente não viu (o botão `ZOC:` não tinha ligação com revisão), a mensagem saía antes de persistir o estado e o clique era marcado como tratado antes do envio, então uma falha de envio deixava pedido criado, cliente não avisado e retry suprimido → o botão carrega a revisão, `resolveConfirmation` recusa confirmar em divergência (reapresenta o resumo atual), e a ordem passou a ser confirmar → persistir → enfileirar, com reconciliação idempotente por `orderingId` — `src/domain/aiWhatsAppOrdering.ts:515`, `server/aiWhatsAppOrdering.ts:1`, `tests/orderingConfirmIntegrity.test.ts:1`
- ✅ IA-WA-019 — não havia recheque local do permit antes das mutações e o caminho de botão mutava antes de checar o envio, então uma tomada humana no meio do turno alterava/cancelava o carrinho sem o cliente saber; `AI_TURN_REVOKED` ainda era retentado com o epoch revogado → `assertPermitCurrent` roda antes de cada mutação e de cada envio, a ordem virou checar → mutar → persistir → enfileirar, e o classificador de confirmação voltou a bater com a lista do CLAUDE.md (`pode`/`show` sozinhos não confirmam; `ok`/`✅`/`👌`/`🙏` voltaram) — `server/aiWhatsAppOrdering.ts:320`, `tests/aiTakeoverRace.test.ts:1`, `tests/aiTurnDecision.test.ts:1`
- ✅ IA-WA-013 — o cursor de mensagens consumidas só avançava num caminho, então a partir do segundo turno a conversa reprocessava o histórico ("sim" depois do resumo virava "talharim\nsim" e caía na IA genérica), o comparador causal ordenava por campos que a mensagem não tem, um áudio falho travava a conversa em "não consegui ouvir" e o quarto turno rápido era descartado em silêncio → cursor persistido em todos os caminhos terminais, comparador nos campos reais, áudio falho consumido uma vez com resposta amigável e turno seguinte normal, e turno excedente é coalescido com retomada garantida em vez de descartado — `server/aiWhatsAppOrdering.ts:360`, `server/orderingTurnComposer.ts:77`, `server/transcription.ts:1`, `server/index.ts:1`
- ✅ IA-WA-020 — o formato da autoridade não tinha artefato compartilhado, então cada repositório inventava o double do outro → o ZeloMenu gera e versiona as fixtures reais em `docs/contracts/conversation-ordering-wire/v1/` (snapshots, catálogo exaustivo de tipos de requisito e de erros, corpos aceitos e rejeitados) com teste de drift, e o ZeloChat copia a pasta verbatim e testa contra ela — `tests/fixtures/zelomenu-wire/v1/README.md:1`, `tests/zeloMenuWireContract.test.ts:1`

### Pedidos conversacionais híbridos (2026-09-03)

- ✅ IA-WA-011 — a leitura do snapshot canônico enviava só `empresaId`, mas a autoridade exige também `remoteJid` (400 `CONVERSA_INVALIDA`); qualquer conversa com ponteiro falharia antes de planejar/confirmar → o cliente interno passa `orderingId + empresaId + remoteJid` (JID codificado na URL) e o handler valida empresa/JID do snapshot contra a conversa local; teste de contrato reproduz o 400 sem JID — `server/zeloMenuInternalClient.ts:175`, `server/aiWhatsAppOrdering.ts:201`, `tests/zeloMenuInternalClient.test.ts:1`
- ✅ IA-WA-012 — a prévia em dry-run do simulador assumia retirada em silêncio quando o cliente ainda não tinha escolhido entrega/retirada e a saudação não deixava explícito o pedido por escrito → a prévia pergunta "entrega ou retirada" antes do resumo e a entrada diz "pode fazer o pedido por escrito ou mandar um áudio" no mesmo payload usado pelo WhatsApp real — `src/domain/aiWhatsAppOrdering.ts:54`, `tests/aiSimulatorOrdering.test.ts:80`
- ✅ TEST-HARNESS-003 — `npm test` tinha 3 arquivos falhando (fixture do composer sem tipo, mock de job sem `payload` no rollout, 4 guardrails apontando para símbolos retirados de `server/router.ts`) → fixture tipada com `TimedComposerMessage`, mock completo e guardrails reescritos contra o pipeline de outbound (`conversationOutbound` / `outbound/worker` / `fromMe`), sem enfraquecer asserções; suíte completa verde — `tests/orderingTurnComposer.test.ts:1`, `tests/customerRollout.test.ts:1`, `tests/auditFixGuardrails.test.ts:1`
...
- ✅ TAKEOVER-012 — o plano de rollout não tinha modo único fail-safe, rollback repetível nem cleanup pós-drain cercado → a revisão deixou os gates explícitos e a decisão posterior (TAKEOVER-014) removeu o rollout da engine, tornando o takeover autenticado sempre ativo — `docs/runbooks/HUMAN_TAKEOVER_OUTBOUND.md:1`, `supabase/migrations/067_conversation_outbound_rolling_cleanup.sql:1`
- ✅ TAKEOVER-013 — a reserva atômica de takeover pausava a IA no banco, mas não avisava as outras abas/sessões da mesma família → o dispatcher agora publica `conversation_mode_changed` tenant/família-scoped com epoch e JIDs canônicos após o primeiro takeover; retries idempotentes não reemitem o evento — `server/conversationOutbound.ts:70`, `supabase/migrations/065_conversation_outbound_claims.sql:1038`, `tests/manualOutboundRoutes.test.ts:1`
- ✅ TAKEOVER-014 — a proteção de takeover ainda tinha rollout `shadow/enforce`, allowlist, kill switch e cleanup condicionado a confirmação → engine nativa autenticada agora é sempre ativa, replay/worker iniciam sem flag, o módulo de rollout foi removido e a migration 067 pode ser aplicada diretamente após o índice tenant-scoped — `server/fromMeProcessor.ts:135`, `server/webhookReplayWorker.ts:96`, `server/outbound/worker.ts:327`, `supabase/migrations/067_conversation_outbound_rolling_cleanup.sql:1`
- ✅ TAKEOVER-015 — as migrations da engine ainda não estavam no banco compartilhado → `063`, `064`, `065`, `066` e `067` aplicadas em ordem no projeto Supabase `ZeloPDV`; verificação SQL confirmou tabelas, RPC nativa, lock gate, índices tenant-scoped e remoção do gate/bridge legados — `supabase/migrations/063_conversation_outbound_foundation.sql:1`, `supabase/migrations/067_conversation_outbound_rolling_cleanup.sql:1`
- ✅ TAKEOVER-016 — remover a trava de rollout faria o replay worker construir o cliente Supabase durante o boot e derrubar ambientes sem credenciais → o worker agora mantém o scheduler vivo e inicializa o acesso ao banco sob demanda, tratando configuração ausente como falha de ciclo — `server/webhookReplayWorker.ts:96`
- ✅ TAKEOVER-017 — o smoke E2E autenticado falhava por seletor ambíguo entre a aba e o submit “Entrar” → o teste agora restringe o clique ao formulário e passou contra `https://chat.zelopdv.com.br` com credencial de teste — `tests/e2e-live-auth.spec.ts:19`
- ✅ TAKEOVER-011 — faltava um gate reproduzível para provar locks, idempotência, takeover nativo e comportamento fim a fim sem tocar produção → probe SQL inteiro em `BEGIN/ROLLBACK`, teste composto/manifesto da matriz de races e E2Es opt-in usam somente `/webhook/:instance`; scripts dedicados permitem rodar os gates sem incluir o legado 410 — `supabase/verification/conversation_outbound_engine.sql:1`, `tests/conversationOutboundIntegration.test.ts:1`, `tests/qa-human-takeover.spec.ts:1`, `tests/qa-native-from-me.spec.ts:1`, `package.json:15`
- ✅ TAKEOVER-011-R1 — review encontrou probe sem prova direta do escopo tenant do `wa_message_id` e E2Es que aceitavam falha/estado anterior → probe agora captura duplicata no mesmo tenant e aceita o mesmo ID em tenant distinto; ambos os E2Es criam JID/IDs exclusivos, forçam e confirmam baseline IA, exigem HTTP 200/202 e provam mensagem/job novos, transição causal para Manual e ausência de escalação — `supabase/verification/conversation_outbound_engine.sql:18`, `tests/qa-human-takeover.spec.ts:13`, `tests/qa-native-from-me.spec.ts:13`
- ✅ TAKEOVER-011-R2 — o E2E do composer provava takeover e persistência, mas não excluía escalação concomitante → a condição terminal agora exige `autoReply=false`, `status='active'`, `escalatedAt=null` e o novo job na mesma leitura — `tests/qa-human-takeover.spec.ts:66`, `tests/conversationOutboundIntegration.test.ts:227`
- ✅ TAKEOVER-010 — o frontend ainda podia exibir sessões irmãs em modos diferentes, perder o ID retornado no `202 queued`, duplicar bolhas por append e oferecer retry ambíguo → evento tipado atualiza toda a família sem reativar IA, `autoReply` é derivado, lifecycle canônico aparece nas duas telas e entrega incerta exige confirmação para “Enviar uma nova cópia”; resposta HTTP e WebSocket fazem upsert pelo ID do backend — `server/ws.ts:5`, `src/hooks/useWhatsAppSessions.ts:120`, `src/components/views/MessageBubble.tsx:640`, `src/components/customers/CustomerMessagesTab.tsx:1`, `tests/conversationModeUiGuardrails.test.ts:1`
- ✅ TAKEOVER-009 — transacionais, campanhas, automações, outreach interno e mensagens do ZeloMenu ainda tinham transportes/upserts paralelos que podiam perder lifecycle, origem ou isolamento da idempotência → produtores agora gravam payload/fingerprint/origem com `preserve_ai`, todo envio visível passa pelo dispatcher/worker, jobs sem sessão são serializados por `empresa + destino` sem criar controle de cliente, e o guardrail global permite direto apenas efeitos de protocolo — `server/router.ts:420`, `server/zelomenuCartSessions.ts:1082`, `server/campaigns/service.ts:72`, `server/automations/sweeper.ts:31`, `supabase/migrations/065_conversation_outbound_claims.sql:200`, `tests/systemOutboundPolicy.test.ts:1`
- ✅ TAKEOVER-009-R1 — review P1 encontrou transacional sem sessão persistido como campanha, sujeito ao rollout CRM desligado → novo `job_type='transactional'` mantém `system_transactional/preserve_ai`, passa pelo mutex tenant+destino e pelo worker sem consultar gate/ledger de campanha; campanhas e automações permanecem inalteradas — `server/outbound/queue.ts:25`, `server/outbound/worker.ts:194`, `supabase/migrations/065_conversation_outbound_claims.sql:1038`, `tests/conversationOutboundWorker.test.ts:164`
- ✅ TAKEOVER-008 — eco `fromMe` e envio humano nativo ainda dependiam de mapa local/fire-and-forget, sem takeover atômico, autenticação de enforce ou replay leased → migration 066 registra mensagem/job nativo + modo/epoch na mesma transação, classificador exige ID persistente para eco, fingerprint idêntico vira hold, raw event precede ACK e falhas autenticadas recebem replay/backoff/dead-letter; o runtime permanece `shadow` por padrão — `server/fromMe.ts:1`, `server/fromMeProcessor.ts:1`, `server/webhookReplayWorker.ts:1`, `supabase/migrations/066_native_from_me_takeover.sql:1`, `tests/fromMeClassifier.test.ts:1`
- ✅ TAKEOVER-008-R1 — review P1 encontrou ACK 200 mesmo sem raw event durável e hold de correlação preso quando o job concluía com ID diferente → webhook retorna 503 amigável antes de processar/ACK, e a RPC nativa libera sob lock somente `from_me_pending_correlation` ligado ao job concluído com outro ID, preservando qualquer hold independente — `server/router.ts:995`, `supabase/migrations/066_native_from_me_takeover.sql:83`, `tests/routerWebhookGuardrails.test.ts:22`, `tests/fromMeProcessor.test.ts:95`
- ✅ TAKEOVER-008-R2 — review P1 encontrou inversão de locks entre hold (`job→control`) e cleanup nativo (`control→job`), com risco de deadlock → hold e reconciliação agora descobrem o controle sem lock e revalidam tenant/job sob a ordem única `gate→control→job`; cleanup continua restrito ao hold de correlação do mesmo job — `supabase/migrations/066_native_from_me_takeover.sql:197`, `tests/fromMeProcessor.test.ts:109`
- ✅ TAKEOVER-007 — envios humanos por `/api/send`, CRM, contato/lista/localização/reação/enquete e retry ainda chamavam transporte direto sem reservar takeover/ledger conversacional → todos agora usam `dispatchConversationOutbound` como `human_zelochat`/`take_over`, passam ator autenticado com `clientes.comunicar`, aceitam idempotency key por intenção e retornam lifecycle discriminado sem erro técnico — `server/router.ts:309`, `server/router.ts:1523`, `server/customers/router.ts:91`, `src/hooks/useWhatsAppSessions.ts:358`, `tests/manualOutboundRoutes.test.ts:1`
- ✅ TAKEOVER-006 — respostas automáticas podiam atravessar takeover durante debounce/model/tools/fallbacks e escalações ainda enviavam direto → `AiTurnPermit` agora acompanha o inbound persistido até todos os helpers/modelos, toda mensagem AI usa o dispatcher com epoch, e handoff/gerente usam jobs `system_handoff`/`internal_system` separados — `server/ai.ts:118`, `server/index.ts:239`, `server/escalation.ts:198`, `server/conversationOutbound.ts:582`, `tests/aiTakeoverRace.test.ts:1`
- ✅ TAKEOVER-006-R1 — review P0/P1 encontrou TOCTOU na escalação, confirmação pending com UUID novo/check separado e validador Pix sem fence pós-modelo → claims/confirm agora são RPCs tenant-scoped sob lock com control+epoch+trigger, insert/delete pending são transacionais/idempotentes e ambas as rotas Pix cercam a chamada externa antes/depois — `server/conversationControl.ts:229`, `server/ai.ts:132`, `server/pixReceiptValidator.ts:57`, `supabase/migrations/065_conversation_outbound_claims.sql:740`, `tests/aiTakeoverRace.test.ts:29`
- ✅ TAKEOVER-005 — rotas/IA ainda não tinham seam único para reservar takeover, job e lifecycle terminal sem expor erro técnico → `dispatchConversationOutbound` orquestra reserva humana idempotente, cancelamento local somente após takeover, upload de mídia após job reservado, supressão de IA sem permit/epoch e espera curta por `sent|failed_before_dispatch|delivery_uncertain`; mensagens carregam origem/ator/job e o worker publica lifecycle com concorrência local limitada por conversa — `server/conversationOutbound.ts:1`, `server/messageHandler.ts:1289`, `server/outbound/worker.ts:255`, `server/index.ts:482`, `tests/conversationOutbound.test.ts:1`
- ✅ TAKEOVER-005-R1 — review P1 encontrou TOCTOU no enqueue AI, corrida de retry na preparação de mídia, debounce repetido em retry idempotente e broadcast WS podendo impedir POST → dispatcher usa RPC AI tenant-scoped/atômica, mídia tem owner/CAS `preparing→queued`, `begin_zelochat_human_outbound` retorna `takeover_applied` real, status broadcast é fail-soft e `failed_before_dispatch` segue retryable no rolling deploy — `server/conversationOutbound.ts:1`, `supabase/migrations/065_conversation_outbound_claims.sql:1`, `server/outbound/worker.ts:1`, `src/domain/outbound.ts:65`, `tests/conversationOutbound.test.ts:1`
- ✅ TAKEOVER-005-R2 — review P1 encontrou reuso divergente de idempotency key AI e fingerprint de mídia insuficiente antes do upload → RPC AI valida job existente contra empresa/JID/controle/epoch/origem e fingerprint/payload esperados, `intent_payload_fingerprint` preserva a intenção original e o claim de mídia exige essa fingerprint antes de adquirir ownership — `server/conversationOutbound.ts:1`, `supabase/migrations/065_conversation_outbound_claims.sql:1`, `tests/conversationOutbound.test.ts:1`, `tests/conversationOutboundWorker.test.ts:369`
- ✅ TAKEOVER-005-R3 — review P1 de rolling deploy apontou que réplicas antigas perderiam o overload de claim de mídia e jobs pre-R2 com intent nula seriam suprimidos → migration preserva o claim 4-args em envelope legado seguro, o claim 5-args adota `intent_payload_fingerprint` uma única vez para retries exatos pre-R2 e payload divergente falha fechado sem upload — `supabase/migrations/065_conversation_outbound_claims.sql:445`, `server/conversationOutbound.ts:150`, `tests/conversationOutbound.test.ts:157`, `tests/conversationOutboundWorker.test.ts:373`
- ✅ TAKEOVER-005-R4 — review P1 mostrou que metadata pre-R2 não prova os bytes de uma mídia humana e permitia ao dispatcher novo adotar intent nula com a mesma idempotency key → retries humanos pre-R2 agora retornam falha amigável exigindo nova tentativa antes de claim/upload, sem alterar o job; o claim novo de 5 args só adota null-intent para AI, enquanto o overload legado de 4 args permanece para réplicas antigas — `server/conversationOutbound.ts:254`, `supabase/migrations/065_conversation_outbound_claims.sql:517`, `tests/conversationOutbound.test.ts:410`, `tests/conversationOutboundWorker.test.ts:393`
- ✅ TAKEOVER-004 — fila conversacional ainda aplicava rollout CRM e permitia leases/finalizações sem mutex canônico entre réplicas → migration 065 serializa claim pelo `conversation_control_id`, suprime epoch obsoleto, transforma lease pós-POST vencida em entrega incerta/hold e fenceia lifecycle pelo owner; worker usa adapter exaustivo, IDs reais e mídia durável sem Data URL no JSONB — `supabase/migrations/065_conversation_outbound_claims.sql:1`, `server/outbound/worker.ts:1`, `server/outbound/providerAdapter.ts:1`, `server/outbound/mediaStore.ts:1`, `tests/conversationOutboundWorker.test.ts:1`
- ✅ TAKEOVER-004-R1 — preflight ainda ocorria depois do fence, deferrals consumiam tentativa e mídia era remontada como Data URL → preparação/checksum/validação agora antecede `startTransport`, apenas o POST ocorre depois do fence, tentativas começam no transporte, mídia usa URL assinada HTTPS com ingest limitado/allowlist e cleanup de produção; lifecycle incerto e lock order foram endurecidos — `server/outbound/worker.ts:1`, `server/outbound/mediaStore.ts:1`, `server/outbound/providerAdapter.ts:1`, `supabase/migrations/064_conversation_control_rpcs.sql:195`, `supabase/migrations/065_conversation_outbound_claims.sql:1`
- ✅ TAKEOVER-004-R2 — takeover podia vencer durante o preflight sem nova validação no fence, controles fundidos podiam perder holds e paths de mídia não eram ligados ao tenant/job → `startTransport` relê modo/epoch/hold sob gate+controle, merge preserva e encadeia ambiguidades, CHECK/runtime exigem payload persistido fail-closed e path exato com bridge temporária somente para writers legados de campanha/automação, request HTTP fica serializada antes do fence e o probe Postgres cobre as duas ordens de lock — `supabase/migrations/064_conversation_control_rpcs.sql:195`, `supabase/migrations/065_conversation_outbound_claims.sql:1`, `server/outbound/providerAdapter.ts:1`, `server/outbound/mediaStore.ts:1`, `tests/conversationOutboundRpc.integration.test.ts:1`
- ✅ TAKEOVER-004-R3 — hold instalado entre claim/preflight e o fence cancelava a intenção humana, e payload nulo retornado pelo banco era promovido silenciosamente a texto → hold agora reverte job/mensagem atomicamente para `queued`, sem tentativa/lease/POST, enquanto somente IA stale é cancelada; mapper/queue removem fallback e claim inválido terminaliza `failed_before_dispatch` antes do provider — `supabase/migrations/065_conversation_outbound_claims.sql:430`, `server/outbound/queue.ts:85`, `server/outbound/worker.ts:17`, `tests/conversationOutboundWorker.test.ts:170`, `tests/outboundQueue.test.ts:27`
- ✅ TAKEOVER-003-R3 — rereview mostrou que ordenar a família dentro do trigger AFTER ROW ainda era tarde demais, pois o UPDATE legado já segurava um row lock arbitrário → migration 064 adiciona gate advisory transacional global de rolling deploy, adquirido por trigger BEFORE STATEMENT de sessões e no início dos RPCs/helpers antes de qualquer row lock; guardrail verifica gate-before-row, chave única e mesma ordem em RPCs/bridge — `supabase/migrations/064_conversation_control_rpcs.sql:73`, `supabase/migrations/064_conversation_control_rpcs.sql:833`, `supabase/migrations/064_conversation_control_rpcs.sql:994`, `tests/conversationControl.test.ts:374`
- ✅ TAKEOVER-003-R2 — rereview apontou deadlock de rolling deploy porque o trigger legado fazia sessão→controle enquanto RPCs novos podiam fazer controle→sessão → migration 064 agora centraliza lock de família de sessão em ordem determinística antes de advisory/control locks, e o bridge legado trava a família antes de tocar controle; guardrail congela a ordem sessão→advisory→controle — `supabase/migrations/064_conversation_control_rpcs.sql:103`, `supabase/migrations/064_conversation_control_rpcs.sql:250`, `supabase/migrations/064_conversation_control_rpcs.sql:830`, `tests/conversationControl.test.ts:372`
- ✅ TAKEOVER-003-R1 — review apontou split `person:*`/`phone:*`, janela de rolling deploy com writes legados, validação tenant ausente, ator do toggle, merge com jobs ativos, locks fora de ordem, epoch number e logs com JID cru → migration 064 agora une pessoa/contact key, usa locks determinísticos, valida ator/mensagem por tenant, bridge de `auto_reply` legado com epoch/auditoria, terminaliza jobs ativos conflitantes antes do merge, route passa ator real, TS rejeita epoch number e logs redigem JID — `supabase/migrations/064_conversation_control_rpcs.sql:43`, `server/conversationControl.ts:80`, `server/router.ts:2527`, `tests/conversationControl.test.ts:123`
