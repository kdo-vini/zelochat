...
|----|-------|--------|-------|-------|
| P0.1 | `/webhook/:instance` no caller authentication | ✅ | `server/router.ts:357`, `server/instanceManager.ts:106`, `server/webhookLog.ts`, `supabase/migrations/017_webhook_events_raw_auth_status.sql` | Validate-if-present mode shipped + `auth_status` tracking on each raw event (Sprint 17). Whatsmiau headers already configured for both empresas. Flip `WEBHOOK_REQUIRE_TOKEN=1` once 24h of `auth_status='token_match'` confirmed via `zelochat_webhook_events_raw`. |
