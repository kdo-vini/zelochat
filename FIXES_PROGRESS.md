...
- ✅ PR-1.1/1.3 — merge do fluxo canônico não podia ser um no-op por empresa: `!isGeneralMode` + `ai_enabled` global + `ZELO_INTERNAL_API_KEY` de processo já eram suficientes para trocar o comportamento de um tenant já ativo no deploy, sem flag pra reverter → nova coluna `empresa_perfil.ai_hybrid_ordering_enabled` (migration 068, default false); `isAiHybridOrderingEnabled()` é a fonte única e gateia o router em `server/ai.ts`, o roteamento de botão/texto canônico em `server/router.ts` e o dry-run do simulador em `server/aiSimulator.ts`; exposta somente-leitura em `GET /api/ai-ordering-status` (sem toggle self-service), refletida no card de Configurações — `supabase/migrations/068_ai_hybrid_ordering_flag.sql`, `server/configStore.ts:507`, `server/ai.ts:3792`, `server/router.ts:642,721`, `server/aiSimulator.ts:144`, `src/domain/aiWhatsAppOrderingUi.ts:1`, `tests/aiHybridOrderingFlag.test.ts:1`
...
- ✅ PR-1.39 — Novidades publicava a entrada `big` "Pedido por mensagem ficou mais fácil" em 2026-09-03 para um recurso que está atrás de uma flag `default false` e não está no ar para tenant nenhum → entrada removida de `src/data/changelog.ts`; volta quando o piloto realmente for ativado para a primeira loja — `src/data/changelog.ts:17`
