...
- ✅ AI-WA-ROLLOUT-REMOVE — havia uma chave por empresa, migration, hidratação isolada, gates duplicados e um terceiro estado na interface para um piloto que nunca foi solicitado; como existe um cliente em produção e não haverá rollout gradual, o fluxo canônico volta a rodar para toda empresa fora do modo `general`, mantendo `ai_enabled` e `ai_mode = 'always_off'` como controles operacionais → migration 068 removida; runtime e status simplificados em `server/configStore.ts`, `server/ai.ts`, `server/router.ts`, `server/aiSimulator.ts` e `src/services/waApi.ts`; card reduzido a integração configurada/não configurada em `src/domain/aiWhatsAppOrderingUi.ts` e `src/components/views/SettingsView.tsx`; cobertura ajustada em `tests/aiHybridOrderingRestaurantMode.test.ts`, `tests/aiSimulatorOrdering.test.ts`, `tests/aiWhatsAppOrderingStatusRoute.test.ts`, `tests/aiWhatsAppOrderingUi.test.ts`, `tests/hybridOrderingScenarios.test.ts` e `tests/routerWebhookGuardrails.test.ts`; documentação atualizada em `CLAUDE.md`, `CURRENT.md` e `docs/ai/ZeloChat.memory.md`.
