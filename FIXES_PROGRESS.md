...
- ✅ AI-WA-ORDERING-006 — a IA só podia enviar o link do cardápio → tracer isolado usa catálogo/ordering internos autenticados, confirmação determinística com token opaco, revisão otimista, resumo curto e botões Confirmar/Alterar; a ativação é global e o modo de teste fica restrito à operação — `server/aiWhatsAppOrdering.ts:1`, `server/zeloMenuInternalClient.ts:1`, `src/domain/aiWhatsAppOrderingUi.ts:1`
- ✅ AI-WA-ORDERING-007 — o status poderia aparecer ativo sem chave/URL interna configurada → endpoint do painel agora só marca a função como ativa quando o modo global está ligado e o cliente privado do ZeloMenu consegue ser criado; falha de leitura fica neutra e não promete atendimento automático — `server/router.ts:1638`, `src/components/views/SettingsView.tsx:483`
...
- ✅ CRM-ORDER-CONTEXT-R1-005 — PATCHes simultâneos podiam sobrescrever campos, horários em torno da meia-noite produziam meio-dia e o último pedido descartava partes dos snapshots → store agora delega merge/tenant ao RPC atômico compartilhado, UI trava todas as ações durante save, horário usa mediana circular e customer/fulfillment/payment completos são preservados — `server/customers/orderingContext.ts:1`, `server/customers/orderingContextAdapter.ts:1`, `src/components/customers/CustomerSummaryTab.tsx:1`
