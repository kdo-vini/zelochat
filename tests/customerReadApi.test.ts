...
// A recent purchase must remain recent when this test runs in future months.
const recentPurchase = new Date(Date.now() - 86400000).toISOString();
const repo = { listPeople: async () => [{ id: 'p1', nome: 'Ana', contato: '5511', updated_at: '2026-01-01' }], countOrders: async () => ({ p1: { count: 1, total: 25, lastDeliveredAt: recentPurchase } }), lastConversations: async () => ({ p1: '2026-08-01T00:00:00Z' }), getPerson: async () => null };
