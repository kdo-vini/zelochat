...
- **Recibo de pedido do ZeloMenu só recebe agradecimento.** `isZeloMenuOrderReceipt` casa o rodapé do template (`Sistema Zelo Menu` + `Pedido #`) e encerra o turno antes de qualquer classificação — o texto contém "cardápio digital" e "Entrega · o quanto antes", então casava com o pedido de cardápio E com a pergunta de taxa. Pergunta depois do recibo sobre o que o pedido inclui ("lá no cardápio não mostra o acompanhamento") não é pedido de cardápio: `isRequestingTheMenu` distingue "me manda o cardápio" de falar *sobre* o cardápio, senão o cliente recebe o cartão de entrada de novo (Bem Servido / Silvia, 2026-09-18).
