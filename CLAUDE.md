...
- **Sem pedido em andamento, o roteador de intenção decide a entrada do texto livre.** Sem carrinho editável, incluindo saudações e respostas curtas, `server/orderingTurnRouter.ts` usa `gpt-4o-mini` com JSON schema estrito, e o domínio puro fica em `src/domain/orderingTurnRoute.ts`; essa decisão não é mais da lista de palavras de `classifyOrderingTurn`. `conversa`/`outro`/`atendente` vão para a IA genérica sem cartão do cardápio; `pedir_cardapio` recebe o cartão; `pedido`/`duvida_cardapio` seguem para o catálogo usando os `items` citados como busca. Confirmação, botões e pedido aberto continuam determinísticos. Falha/timeout (3 s, `ZELOCHAT_ORDERING_ROUTER_TIMEOUT_MS`) ou `ZELOCHAT_ORDERING_ROUTER=0`/`false`/`off` retoma o caminho legado; a falha do roteador não transfere diretamente, mas falhas posteriores do catálogo/pedido ainda usam a recuperação existente. O incidente de 24/09/2026 foi uma pesquisa de satisfação com "Queria te pedir uma ajuda", que recebeu "Não consegui conferir o pedido com segurança agora; vou chamar um atendente": `pedir` + frase de ~700 caracteres foi rejeitado pela busca do ZeloMenu (limite 240, hoje `clampCatalogQuery`). Se o prompt do roteador mudar, rode `npx tsx scripts/evalOrderingRouter.ts` (precisa de `OPENAI_API_KEY`) e só publique com `GATE: PASS`; `tests/fixtures/ordering-router-eval/v1.jsonl` é gabarito rotulado por humano — não edite rótulos para fazer passar nem cole frases dele no prompt. Referência de 24/09: decisão 100% (palavra-chave 70,7%), 0 pedidos falsos, 0 perdidos, p50 ~0,75 s. A `confidence` declarada pelo modelo não é calibrada (0,9 em acertos e erros na primeira rodada); o corte de 0,6 é só rede de segurança, não serve para decisões novas. Em produção, observe `[AiOrderingMetric]` no stage `route` (outcomes: nome da intenção, `router_failed`, `router_disabled`) e o outcome de plano `routed_generic`.
- **Revisão de produto de 24/09/2026:** saudações e respostas curtas sem carrinho passam pelo roteador com histórico; ponteiro de pedido encerrado é consultado antes da decisão e não impede nova conversa. Confirmação/cancelamento continuam determinísticos. `duvida_cardapio` não autoriza planner por conter “quero”; responde preços/opções canônicos e explicita quando composição não está disponível. Pedido sem itens pede esclarecimento.
- **Busca com vários itens:** `server/orderingCatalogSearch.ts` complementa a consulta composta com consultas individuais, até três simultâneas. O ranking por cobertura pode perder todos os pratos ao unir três nomes. Alternativas para um mesmo item continuam ambíguas; candidatos ficam limitados a 12. Falha de uma consulta não produz carrinho parcial silencioso.
- **Gate de publicação do roteador:** executar `scripts/evalOrderingRouter.ts` com a referência e com `tests/fixtures/ordering-router-eval/challenge-v1.jsonl`. A referência de 75 casos foi usada para ajustar o prompt; seus 100% históricos não validam o prompt revisado. Falhas contam como `fallback_unknown` e bloqueiam o gate. Esta avaliação mede roteamento, não resposta final, extração de todos os itens ou qualidade do planner. Evidências e pendências em `docs/audits/2026-09-24-ordering-router-review.md`.
...
- **Fallback sem decisão do roteador: o catálogo mantém a entrada de pedidos sem palavras-chave.** O modelo genérico de `server/ai.ts` não tem ferramenta de pedido: todo turno de pedido que cai nele vira resumo inventado (foi assim que "Macarrão, pene" e um áudio listando penne, molho e azeitona receberam "Retirada: Amanhã" num pedido do mesmo dia, 14/09/2026). No fallback legado (roteador desativado ou falhou): (1) se `classifyOrderingTurn` diz `none`, o fluxo consulta o catálogo com a frase do cliente e entra quando há resultado — o ZeloMenu descarta palavra que a loja não vende, então "obrigada" e "me manda o endereço" voltam vazios; saudação exata e pedido de atendente não consultam; (2) falha dessa consulta cai no genérico em silêncio, nunca em transferência; (3) citar o que a loja vende sem perguntar (`isOrderingQuestion`, que ignora "tá?"/"né?" no fim da frase) é pedido, com ou sem "quero"; (4) várias opções do **mesmo** produto não são ambiguidade (`isSingleProductCatalogAmbiguous`) — pratos diferentes continuam perguntando qual, **exceto** quando um token da frase do cliente aparece no nome de um único prato (`resolveNamedCatalogMatch`: "Marmita média bife a cavalo" não pergunta se era pizzaolo); (5) turno que só o catálogo deixou entrar e do qual o planner não tirou carrinho volta ao genérico em vez de mandar lista de produtos — **também quando o planner nem rodou** porque o hit é ambíguo e nenhum token da frase aparece no nome do prato (`catalogQueryNamesAListedProduct`: "safada" não lista Salada; "Macarrão,pene" ainda pergunta qual); (6) o histórico do planner usa a transcrição do áudio (`buildPlannerHistory`). **Não "corrija" pedido inventado apertando o prompt genérico para mandar o cliente ao cardápio** — o defeito é sempre o turno não ter chegado aqui. Cenário 11 de `tests/hybridOrderingScenarios.test.ts` usa as mensagens reais.
...
- **Recibo de pedido do ZeloMenu só recebe agradecimento.** `isZeloMenuOrderReceipt` casa o rodapé do template (`Sistema Zelo Menu` + `Pedido #`) e encerra o turno antes de qualquer classificação — o texto contém "cardápio digital" e "Entrega · o quanto antes", então casava com o pedido de cardápio E com a pergunta de taxa. **Caminho canônico monta rascunho; modelo genérico conversa.** Pedido **só** de cardápio (`isMenuOnlyRequest`: nomeou o menu e, sem `cardápio`/`menu`/`lista`, o classificador é `none`) responde o cartão e **nunca** busca, abre rascunho nem escala — "Olá boa noite / Cardápio por favor" não pode virar "vou chamar um atendente" (Aline, 19/09). Pergunta sobre o que o pedido já inclui ("esse pedido vem arroz?", "lá no cardápio não mostra o acompanhamento", "Qdo ficar pronto") cai no genérico (`isTalkingAboutPlacedOrder` / `isRequestingTheMenu`): `pedido` e `arroz` são palavras do classificador e um hit no catálogo virava lista ou carrinho novo. Extra em carrinho aberto ("Adicional mandioca") é `alter`, não busca. Pedido novo ("quero fazer um pedido") continua no assembler. Não mande pedido nomeado para o genérico — ele inventa resumo (14/09).
- **IA não fala por cima de quem acabou de escrever no WhatsApp.** `hasRecentHumanOutbound` silencia o turno por 120s depois da última mensagem `human_native_whatsapp` / `human_zelochat`, mesmo se o modo voltou para `ai` por resume explícito (Simone, 19/09). Rechecado no começo de `generateAndSendReply`, em `enqueueAutomatedText` **e em `dispatchAiPayload`** (lista "Tem sim:", cartão e resumo canônico passam por aqui, não pelo enqueue). Se o envio nativo ainda não chegou no webhook, o hold não vê a linha — classe conhecida de fromMe atrasado.
...
ZELOCHAT_ORDERING_ROUTER         # roteador de intenção ligado por padrão; 0/false/off desliga sem deploy
ZELOCHAT_ORDERING_ROUTER_TIMEOUT_MS # timeout do roteador (default 3000, limitado entre 500 e 8000)
