...
- **Entrega:** ZeloChat `codex/clientes-crm` em `bd762c0` (inclui a limpeza da spec temporária, dados de opt-out/relacionamento, hardening de índices e documentação); migrations compartilhadas no ZeloPDV `codex/clientes-crm` em `8e40e4c`. A migration 060 foi aplicada via Supabase MCP; nenhum `db push`, reparo de ledger ou deploy foi executado.
