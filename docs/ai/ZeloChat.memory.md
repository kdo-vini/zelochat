...
- Customer ordering context boundary (confirmed 2026-08-30): `server/customers/orderingContext.ts` derives a deterministic snapshot from at most 20 tenant/person-scoped committed canonical orders (`accepted|preparing|ready|out_for_delivery|delivered`) and nested canonical items. Fulfillment, address and payment resolve `fixed override > last order > none`; habitual time and recurrence use medians; frequent products are suggestions only and never automatic defaults. The production adapter reads `zelo_orders`/`zelo_order_items`, never `zelochat_orders`; the allowlisted PATCH persists only four removable fields in the customer relationship after `pessoas.gerenciar` and owner/tenant validation.
