...
- Customer ordering context boundary (confirmed 2026-08-30, review R1): `server/customers/orderingContext.ts` derives a deterministic snapshot from at most 20 tenant/person-scoped committed canonical orders (`accepted|preparing|ready|out_for_delivery|delivered`) and nested canonical items. Fulfillment, address and payment resolve `fixed override > last order > none`; habitual time uses a circular median across midnight and recurrence uses interval median; the last order retains customer plus complete fulfillment/payment snapshots and items. Frequent products are suggestions only and never automatic defaults. The production adapter reads `zelo_orders`/`zelo_order_items`, never `zelochat_orders`; the allowlisted PATCH delegates tenant validation and JSON merge atomically to the ZeloPDV-owned service-role RPC `patch_zelochat_customer_ordering_overrides`, never to a process-local lock or client read/merge/upsert.
