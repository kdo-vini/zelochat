...
- Task 4 did not implement or migrate the dispatcher/callers from Task 5. Migration 065 keeps a temporary `zelochat_outbound_job_rolling_bridge` only for old campaign/automation inserts that omit origin/payload; conversation rows never receive defaults. The runtime mapper/queue also never synthesize a text payload from a null/malformed claimed row: the lease is terminalized `failed_before_dispatch` before provider preparation. Remove the rolling bridge only after pre-Task-4 replicas drain. `tests/conversationOutboundRpc.integration.test.ts` provides the real PostgreSQL interleaving probe but explicitly skips without `LOCAL_OUTBOUND_TEST_DATABASE_URL`; keep enforcement rollout gated until it passes locally, dispatcher integration lands, and `fromMe` classification/reconciliation are implemented and verified.
