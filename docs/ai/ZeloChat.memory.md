...
### Webhook raw e Disk I/O — 2026-09-18 / 2026-09-25
...
- Retenção **não** é job do ZeloChat. Desde 2026-09-25 o dono é o pg_cron `purge-zelochat-webhook-events-raw` (ZeloPDV #53): RPC `purge_zelochat_webhook_events_raw_batch(interval, int)`, só `service_role`, lote máx. 500, keep 3 dias, só processadas. Nunca reintroduzir `.from('zelochat_webhook_events_raw').delete()` nem um segundo sweeper no backend. Purge de tenant em `delete_account` (por `empresa_id`) não é retenção e permanece no ZeloPDV.
...
- Task 8 adds migration `066_native_from_me_takeover.sql` plus `fromMeProcessor`: provider/message ID is definitive echo evidence; an equal fingerprint on a `dispatch_started` job without provider ID creates a durable correlation hold and replay, never an echo verdict. Authenticated native sends insert the `human_native_whatsapp` message/job and apply takeover in one transaction; raw webhook persistence precedes ACK, failed processing remains unprocessed for leased retry/backoff/dead-letter, and missing/mismatched tokens never mutate conversation state. `FROM_ME_NATIVE_MODE` defaults to shadow and the replay loop does not start outside enforce. Keep enforce off until migration 066 is applied, real media fixtures are validated and Task 9/rollout gates are complete.
- Task 10 makes `conversation_mode_changed` the realtime frontend authority for an entire canonical session family. The hook stores epoch as a string, derives legacy `autoReply` from the mode, clears takeover metadata only on explicit resume, normalizes rolling `failed` to `failed_before_dispatch`, and upserts optimistic/WS messages by the backend message ID. Delivery uncertainty is never a blind retry: the operator must explicitly confirm “Enviar uma nova cópia”, and queued messages behind an observed uncertain delivery explain the hold without claiming that the earlier send was cancelled.
- Task 11 adds the release-evidence harness without changing runtime behavior: `supabase/verification/conversation_outbound_engine.sql` is an isolated-database-only `BEGIN/ROLLBACK` probe for grants, tenant actor checks, epoch/family projection, stale AI cancellation, idempotent human/native writes and single claim; `npm run test:takeover` is the focused deterministic gate. The two Playwright takeover specs are opt-in and require explicit isolated backend credentials. Passing locally without those credentials means “skipped”, never evidence for enabling `FROM_ME_NATIVE_MODE=enforce`.
- Task 12 adds the server-side rollout authority in `server/outbound/rollout.ts`: missing/invalid config is `shadow`, a tenant allowlist can stage `enforce`, and the legacy `FROM_ME_NATIVE_MODE` remains only for rolling compatibility. Authenticated `fromMe` shadow performs evidence reads plus pure classification and emits only redacted `would_takeover|would_correlate` metrics; it never calls reconciliation/takeover RPCs, cancels timers, creates jobs/messages or broadcasts mode. `server/outbound/observability.ts` also tracks human takeover, stale AI suppression, native/echo/correlation decisions, delivery uncertainty, queue age/depth and stuck leases without content/phone/JID leakage. `docs/runbooks/HUMAN_TAKEOVER_OUTBOUND.md` is the gate/rollback authority. Migration 067 is forward-only, guarded by an explicit drain setting, removes the temporary global idempotency/status/bridge compatibility only after all old replicas drain, and was not applied live.
- Final transversal review (2026-08-31) found and closed a P1: `begin_zelochat_human_outbound` now returns the canonical family JIDs/epoch/timestamp and `dispatchConversationOutbound` emits one tenant-scoped `conversation_mode_changed` event after the first applied takeover. This keeps other tabs and sibling sessions in Manual without re-emitting on idempotent retries; the event is fail-soft after the durable DB transition.
- Decisão final (2026-08-31): o rollout `shadow/enforce` foi removido por solicitação do produto. `fromMe` autenticado é processado diretamente, o replay worker sempre inicia, não há allowlist/kill switch de rollout e a migration 067 aposenta o bridge legado. Os bullets anteriores de shadow/enforce descrevem o desenho intermediário e não são mais o comportamento atual.
- Aplicação confirmada (2026-08-31): migrations `063–067` foram aplicadas em ordem no projeto Supabase `ZeloPDV` (`xnnjyrblpvsqrtsshawa`). A consulta de verificação confirmou `zelochat_conversation_control_lock_gate` e `record_zelochat_native_outbound_takeover`, os índices `(empresa_id, wa_message_id)` e `(empresa_id, idempotency_key)`, e a ausência do gate/constraint/trigger legados.
- Robustez pós-rollout (2026-08-31): o replay worker é sempre agendado, mas cria suas dependências Supabase de forma lazy/fail-soft; ausência temporária de credenciais não derruba o processo nem constitui uma decisão de ativação da feature.
- Hotfix nativo confirmado (2026-08-31): a migration `20260831205926_fix_native_from_me_persistence.sql` corrige o contrato incompatível da RPC 066 com `zelochat_outbound_jobs_conversation_shape_check`, exigindo e persistindo `payload_fingerprint`/`intent_payload_fingerprint` e separando o conteúdo exibível do payload terminal do job. `server/fromMe.ts` reconhece a forma real `message.base64` no webhook imediato e, no replay sanitizado, recupera `message.mediaUrl` somente por HTTPS/allowlist com timeout e limite de 25 MB; a mensagem usa anexo estruturado e o job nativo já enviado permanece terminal, sem reenvio.
- Recovery Bem Servido (2026-08-31): após o deploy `160dacf`, os oito eventos `fromMe` das 17:45–17:49 BRT foram reabertos de forma exata e todos processaram na primeira tentativa. O banco confirmou oito mensagens/jobs `human_native_whatsapp`, fingerprints de 64 caracteres, três mídias estruturadas e controle `human`/`auto_reply=false`, sem dead-letter.
...
- CRM cursor hardening (confirmed 2026-09-10): timeline/message/order cursors accept the PostgreSQL timestamp forms actually emitted in production (`+00:00`, optional 1–6 fractional digits) without normalizing away sub-millisecond precision; the anchored syntax guard remains mandatory because timeline values enter PostgREST filters. Customer detail order pagination consumes the server-issued cursor/`hasMore`, and recent-customer list cursors carry the raw timestamp through the RPC parameter.
...
## Confirmado 2026-08-31: paridade do simulador de atendimento

- O simulador (`server/aiSimulator.ts`) agora constrói uma sessão efêmera e, em empresas de restaurante, chama `tryHandleAiWhatsAppOrdering` com `dryRun=true` antes do modelo genérico. O dry-run pode ler o catálogo e planejar um preview, mas não chama dispatcher, não persiste ponteiro, não altera/confirma/cancela carrinho e não escala uma conversa real.
- `SimulateDependencies` existe somente para testes focados: injeta a fronteira autenticada do catálogo e um planner determinístico. O caminho padrão continua resolvendo o cliente interno por ambiente; quando a integração não está configurada, o simulador recua para o fallback genérico sem fingir disponibilidade.
- A resposta canônica cobre entrada com link público + pedido escrito, regras reais de cardinalidade (incluindo grupos opcionais), todas as opções retornadas, preview sem preço inventado, taxa de entrega sem valor fabricado e handoff humano customer-facing. `wouldCreateOrder` continua sempre `false`.

## E2E da jornada cliente (2026-08-31)

- O harness do gate completo está em `tests/e2e-ai-customer-journey.spec.ts` e cobre webhook → IA/link → checkout público pela UI → pedido canônico → CRM → status `preparing` → entrega transacional. Ele ainda não foi executado contra serviços reais: é opt-in, exige frontend/API separados, rejeita explicitamente `chat.zelopdv.com.br` e o ref compartilhado `xnnjyrblpvsqrtsshawa`, bloqueia as chamadas do painel durante o login e confere project ref/tenant antes de liberar a jornada.
- O checkout ligado pela Supabase CLI é o projeto compartilhado `ZeloPDV`; não é sandbox. O gate completo precisa de Supabase/tenant/instância/número descartáveis porque as APIs normais não removem raw webhooks, carrinhos, jobs e eventos históricos.
- A conta de login E2E passou no smoke publicado em 2026-08-31, mas isso prova apenas autenticação. Não autoriza usar clientes, catálogo, estoque ou WhatsApp de produção como fixture.
- Risco atualizado durante a auditoria: produção e simulador entram pelo roteador canônico antes do modelo genérico em modo restaurante. Em 2026-09-04, ZeloMenu e ZeloChat foram republicados em ordem; o backend confirmou no boot a URL interna e a chave compartilhada, o card publicado da Bem Servido passou a “Ativo”, e o Supabase confirmou `restaurant` + `ai_enabled=true` + `always_on`. A primeira ordem real controlada ainda é o gate final. Qualquer alteração futura deve preservar o dispatcher durável/takeover no caminho real e o `dryRun` no simulador.
