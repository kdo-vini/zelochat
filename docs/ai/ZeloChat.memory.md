...
- ZeloMenu cart-session backend:
  ...
  - ZLM-201 partial (2026-06-23): the authenticated `Cardápio` view now has a ZeloMenu publication readiness panel backed by `src/domain/zelomenuPublication.ts`. It derives today's safe public-menu status from existing runtime truth: hidden products are inactive, stock-controlled zero-stock products are blocked, and uncategorized products need organization because the public cart serves catalog groups from `catalogHierarchy`. This is a preview/foundation only. Public name, description, photo, public ordering and modifier groups/options remain blocked until the PDV-owned publication overlay from `ZELOMENU_LINEAR_PLAN.md` ZLM-004 is implemented outside this repo.
- AI behavior rules:
