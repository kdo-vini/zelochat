...
- Zelo PDV shared data/integration:
  ...
  - ZeloMenu product publication now uses the real `zelomenu_product_publications` overlay in ZeloChat: `src/hooks/useCatalog.ts` reads/writes it in the authenticated `Cardápio`, and `server/configStore.ts` resolves public catalog items through it before the cart/AI see availability. Modifier groups/options still exist only at schema level and are not yet surfaced in UI/runtime.
...
- ZeloMenu cart-session backend:
  ...
  - ZLM-201 partial (2026-06-23): the authenticated `Cardápio` view now has a ZeloMenu publication panel backed by `src/domain/zelomenuPublication.ts` and the real `zelomenu_product_publications` table. Operators can publish/unpublish, pause temporarily, set public name, description, photo URL and order. The public cart catalog is resolved through the same domain rule, so only published, unpaused, base-valid products become available and public presentation fields come from the overlay. Remaining gap: modifier groups/options and owned image upload/storage.
- AI behavior rules:
...
- ZLM-201 publicação real parcial (2026-06-23): ZeloChat `Cardápio` consome `zelomenu_product_publications` para publicar/despublicar, pausar, configurar nome/descrição/foto URL/ordem. `server/configStore.ts` resolve o catálogo público com `resolveZeloMenuPublicationCatalogProduct`, mantendo preço em `produtos.preco` e filtrando itens não publicados/pausados/inválidos. Modifiers (`zelomenu_modifier_groups`/`zelomenu_modifier_options`) e upload/ownership de imagem continuam pendentes.
