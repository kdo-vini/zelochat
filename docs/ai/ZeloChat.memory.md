...
- Risco atualizado durante a auditoria: produção e simulador agora entram pelo roteador canônico antes do modelo genérico em modo restaurante, mas a IA da Bem Servido permanece desligada até o backend publicado e a integração autenticada do ZeloMenu passarem por conversa controlada; qualquer alteração futura deve preservar o dispatcher durável/takeover no caminho real e o `dryRun` no simulador.
