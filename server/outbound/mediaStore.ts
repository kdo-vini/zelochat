...
export interface OutboundMediaBinding { empresaId: string; jobId: string }
...
export function assertOutboundMediaBinding(payload: PersistedMedia, binding: OutboundMediaBinding): void {
  const expected = `outbound/${binding.empresaId}/${binding.jobId}/${payload.checksum}`;
  if (payload.storagePath !== expected || !/^[a-f0-9]{64}$/.test(payload.checksum)) throw new Error('OUTBOUND_MEDIA_STORAGE_PATH_INVALID');
}
...
export function createOutboundMediaStore(storage: OutboundMediaStorage = defaultStorage()) {
  return {
    ...
    async prepare(payload: PersistedMedia, binding: OutboundMediaBinding): Promise<PreparedOutboundMedia> {
      assertOutboundMediaBinding(payload, binding);
    ...
    async cleanupTerminal(payload: PersistedMedia, binding: OutboundMediaBinding, timing: { terminalAtMs: number; graceMs: number; nowMs?: number }): Promise<boolean> { if ((timing.nowMs ?? Date.now()) < timing.terminalAtMs + timing.graceMs) return false; assertOutboundMediaBinding(payload, binding); await storage.remove(payload.storagePath); return true; },
  };
}
