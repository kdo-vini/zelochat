...
customerRouter.post('/api/customers/:personId/messages', async (req, res) => {
  try {
    ...
    const session = detail?.sessions.find((item) => item.id === jid && /^\d{10,15}@s\.whatsapp\.net$/u.test(item.id));
