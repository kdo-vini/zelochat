...
export async function scheduleCampaign(empresaId: string, ownerUserId: string, id: string, scheduledAt: string | null): Promise<{ campaign: unknown; recipientCount: number }> {
  ...
  if (eligibleRecipients.length) {
    const jobs = eligibleRecipients.map((recipient) => ({ empresa_id: empresaId, campaign_id: id, recipient_id: undefined, pessoa_id: recipient.pessoa_id, job_type: 'campaign', idempotency_key: `${id}:${recipient.pessoa_id}`, instance_key: instanceKey, phone_snapshot: recipient.phone_snapshot, message: interpolateCampaignMessage(campaign.message, recipient.name_snapshot), status: 'queued', next_attempt_at: targetAt.toISOString() }));
    ...
    const { error: jobsError } = await db.from('zelochat_outbound_jobs').upsert(jobs, { onConflict: 'idempotency_key', ignoreDuplicates: true }); if (jobsError) throw jobsError;
  }
...
export async function enqueueCampaignTest(empresaId: string, actorId: string, id: string, requestedPhone?: string): Promise<unknown> {
  const db = getServiceSupabase(); const { data: campaign, error: campaignError } = await db.from('zelochat_campaigns').select('message,status').eq('empresa_id', empresaId).eq('id', id).maybeSingle(); if (campaignError) throw campaignError; if (!campaign || campaign.status === 'cancelled') throw new Error('CAMPAIGN_NOT_FOUND'); const fallback = getOwnJid().split('@')[0] ?? ''; const phone = validateTestPhone(requestedPhone?.trim() || fallback); const instanceKey = await getInstanceForEmpresa(empresaId); const idempotencyKey = `campaign-test:${id}:${actorId}:${Math.floor(Date.now() / 60_000)}`; const { data: job, error } = await db.from('zelochat_outbound_jobs').upsert({ empresa_id: empresaId, campaign_id: id, job_type: 'campaign', idempotency_key: idempotencyKey, instance_key: instanceKey, phone_snapshot: phone, message: interpolateCampaignMessage(campaign.message, 'teste'), status: 'queued', next_attempt_at: new Date().toISOString() }, { onConflict: 'idempotency_key', ignoreDuplicates: true }).select('id,status').maybeSingle(); if (error) throw error; if (!job) throw new Error('CAMPAIGN_TEST_ALREADY_QUEUED'); return job;
}
