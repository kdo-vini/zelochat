...
export function CustomerOrdersTab({ customer, token }: { customer: CustomerDetail; token: string | null }) {
  ...
  return <div className="p-4"><div className="mb-4 grid grid-cols-3 gap-2 text-center"><div className="rounded-lg bg-[var(--color-surface-muted)] p-2"><strong className="block text-[15px] text-[var(--color-ink)]">{customer.orderCount}</strong><span className="text-[11px] text-[var(--color-ink-faint)]">Pedidos entregues</span></div><div className="rounded-lg bg-[var(--color-surface-muted)] p-2"><strong className="block text-[15px] text-[var(--color-ink)]">R$ {customer.totalValue.toFixed(2).replace('.', ',')}</strong><span className="text-[11px] text-[var(--color-ink-faint)]">Total entregue</span></div><div className="rounded-lg bg-[var(--color-surface-muted)] p-2"><strong className="block text-[15px] text-[var(--color-ink)]">R$ {average.toFixed(2).replace('.', ',')}</strong><span className="text-[11px] text-[var(--color-ink-faint)]">Ticket médio</span></div></div>{error && <p role="alert" className="mb-3 text-[12px] text-[var(--color-alert)]">{error}</p>}{orders.length === 0 ? <p className="text-[13px] text-[var(--color-ink-muted)]">Nenhum pedido vinculado.</p> : <><ul className="divide-y divide-[var(--color-line)]">{orders.map((order) => <li key={order.id} className="flex items-center justify-between py-3 text-[13px]"><span className="text-[var(--color-ink-muted)]">{new Date(order.createdAt).toLocaleDateString('pt-BR')} · {orderStatusLabel(order.status)}</span><strong className="text-[var(--color-ink)]">R$ {order.total.toFixed(2).replace('.', ',')}</strong></li>)}</ul>{hasMore && <button type="button" onClick={() => void loadMore()} disabled={loadingMore} className="m-3 min-h-[44px] w-[calc(100%-1.5rem)] rounded-lg border border-[var(--color-line)] text-[13px] text-[var(--color-ink-muted)] hover:bg-[var(--color-surface-muted)] disabled:opacity-50">{loadingMore ? 'Carregando…' : 'Carregar mais'}</button>}</>}</div>;
}
