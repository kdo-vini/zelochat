...
function OrderingHabits({
  ...
  return <section aria-labelledby="customer-ordering-habits-title" className="space-y-3">
    ...
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
      <HabitCard label="Tipo de atendimento" value={typeValue} source={context.fulfillmentType.source} overrideKey="fulfillmentType" overrideValue={context.fulfillmentType.value ?? 'delivery'} actionLabel="tipo de atendimento" canManage={canManage} saving={saving !== null} onChange={update} />
      <HabitCard label="Endereço de entrega" value={address?.display ?? 'Não identificado'} source={context.deliveryAddress.source} overrideKey="deliveryAddress" overrideValue={addressOverride ?? { address: '' }} actionLabel="endereço de entrega" canManage={canManage} saving={saving !== null} onChange={update} />
      <HabitCard label="Forma de pagamento" value={context.paymentMethod.value ?? 'Não identificado'} source={context.paymentMethod.source} overrideKey="paymentMethod" overrideValue={context.paymentMethod.value ?? ''} actionLabel="forma de pagamento" canManage={canManage} saving={saving !== null} onChange={update} />
      <HabitCard label="Horário habitual" value={context.habitualTime.value?.label ?? 'Não identificado'} source={context.habitualTime.source} overrideKey="habitualTime" overrideValue={context.habitualTime.value?.label ?? ''} actionLabel="horário habitual" canManage={canManage} saving={saving !== null} onChange={update} />
    </div>
...
export function CustomerSummaryTab({ customer, canManage = false, onUpdateOrderingOverrides }: CustomerSummaryTabProps) {
  return <div className="space-y-4 p-4">
    ...
    <section><h3 className="mb-1 text-[12px] font-semibold uppercase tracking-wide text-[var(--color-ink-faint)]">Notas internas</h3><p className="whitespace-pre-wrap text-[13px] text-[var(--color-ink)]">{customer.notes?.trim() ? customer.notes : 'Nenhuma nota interna.'}</p></section>
