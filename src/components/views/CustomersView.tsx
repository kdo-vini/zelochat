...
import { CustomerSegmentChips } from '../customers/CustomerSegmentChips';
...
export function CustomersView({ token, onOpenAtendimento, customerPermissions = { pessoasVisualizar: false, clientesComunicar: false }, canManageCustomers = false }: Props) {
  ...
  const [segmentCountsReloadKey, setSegmentCountsReloadKey] = useState(0);
  const { customers, loading, loadingMore, error, hasMore, total, refresh, loadMore } = useCustomers(token, filters);
  const reloadCustomers = async () => { await refresh(); setSegmentCountsReloadKey((value) => value + 1); };
  ...
  return (
    <section className="relative flex min-h-0 flex-1 flex-col bg-[var(--color-canvas)]" aria-label="Clientes">
      ...
      <CustomerSegmentChips area={area} token={token} filters={filters} onChange={setFilters} reloadKey={segmentCountsReloadKey} />
      ...
      {creating && <CustomerCreateDialog onClose={() => setCreating(false)} onCreate={async (patch) => { if (!token) return; await createCustomer(token, patch); setCreating(false); await reloadCustomers(); }} />}</>}
    </section>
  );
}
