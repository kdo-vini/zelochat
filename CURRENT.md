...
Fatias sugeridas:
1. Completar `ZLM-201` com modifiers/adicionais/variações e ownership de imagem, sem alterar `produtos`
...
- ZLM-201 parcial (2026-06-23): o ZeloChat agora consome e grava a camada real `zelomenu_product_publications` no `Cardápio`. O operador configura publicar/despublicar, pausar temporariamente, nome público, descrição, foto por link e ordem; o carrinho público passa a respeitar esse overlay e usa nome/descrição/foto/ordem publicados sem alterar `produtos` nem duplicar preço base. Ainda faltam modifiers/adicionais/variações (`zelomenu_modifier_groups`/`zelomenu_modifier_options`) e ownership/upload de imagem para fechar o aceite completo.
