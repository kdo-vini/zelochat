...
- Rollout Supabase ZeloMenu (2026-06-23) — **parcial**: hardening `zelochat_fix_rls_gaps_2026_06_23` e `trial_expired_status_2026_06_17` aplicados; estruturas `zelomenu_cart_sessions`/`zelomenu_cart_tokens` criadas e verificadas via API; aplicação/validação completa das policies/grants do carrinho e a migration PDV-owned de publicação (`zelomenu_product_publications`/modifiers) ficaram pendentes porque o conector Supabase passou a retornar HTTP 500/-32603 até em `select 1`.
...
2. Desbloquear `ZLM-201` no repo ZeloPDV — camada PDV-owned de publicação/modifiers do ZeloMenu
3. Completar `ZLM-201` no ZeloChat consumindo a camada real de publicação
...
- ZLM-201 parcial (2026-06-23): o ZeloChat ganhou um painel de prontidão de publicação no `Cardápio`, com contagem de produtos prontos para o link, inativos, sem estoque e sem categoria, mais lista acionável para corrigir itens. A regra fica em domínio puro (`src/domain/zelomenuPublication.ts`) e não cria schema conflitante: nome público, descrição, foto, ordem pública e modifiers seguem bloqueados até a camada de publicação PDV-owned nascer no repo ZeloPDV. Próxima fatia recomendada: `ZLM-205` ou migration/adapter de publicação no ZeloPDV.
- Rollout Supabase parcial (2026-06-23): aplicado `zelochat_fix_rls_gaps_2026_06_23` (storage/RPC hardening) e registrado `trial_expired_status_2026_06_17`. A estrutura do carrinho ZeloMenu foi aplicada em `zelomenu_cart_sessions_tables_2026_06_23` e verificada via service role com contagem 0/0. O conector Supabase caiu antes de concluir/verificar todas as policies/grants do carrinho e antes de aplicar a camada PDV-owned de publicação; não considerar o rollout de migrations concluído até finalizar esses dois pontos.
